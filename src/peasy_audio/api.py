"""peasy-audio API client — Access Peasy Audio tools via REST API.

Usage::

    from peasy_audio.api import PeasyAudioAPI

    api = PeasyAudioAPI()
    tools = api.list_tools()          # paginated: {"count", "next", "results": [...]}
    tool = api.get_tool("audio-convert")
"""
...
class PeasyAudioAPI:
    """REST API client for peasyaudio.com.

    All list methods return DRF-paginated responses::

        {"count": 18, "next": "...?page=2", "previous": null, "results": [...]}

    Use ``page`` and ``limit`` to paginate through results.
    """

    def __init__(self, base_url: str = "https://peasyaudio.com") -> None:
